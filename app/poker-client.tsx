...
export default function PokerClient({ user, initialRoomCode }: ClientProps) {
  ...
  useEffect(() => {
    const bb = Math.max(1, game?.room.bigBlind ?? 1);
    const display = amountUnit === "bb" ? raiseAmount / bb : raiseAmount;
    setDirectInput(raiseAmount > 0 ? String(Math.round(display * 100) / 100) : "");
  }, [raiseAmount, amountUnit, game?.room.bigBlind]);
  ...
  function setDirectAmount(raw: string) {
    setDirectInput(raw);
    const value = Number(raw);
    if (!raw || Number.isNaN(value) || value <= 0) return;
    const bb = Math.max(1, game?.room.bigBlind ?? 1);
    const chips = amountUnit === "bb" ? Math.round(value * bb) : Math.round(value);
    if (!game || !game.validActions.canRaise) return;
    const clamped = Math.max(game.validActions.minRaiseTo, Math.min(game.validActions.maxRaiseTo, chips));
    setRaiseAmount(clamped);
    if (game.pot > 0) {
      setPotPercent(Math.max(1, Math.min(300, Math.round((clamped / game.pot) * 100))));
    }
  }
    ...
    <main className="app-shell">
      ...
      <div className="game-layout" ref={layoutRef}>
        <section className="table-panel" aria-label="实时德州扑克牌桌">
          ...
          <div className={`table-log ${logExpanded ? "open" : ""}`}>
            <button type="button" className="table-log-head" onClick={() => setLogExpanded((value) => !value)} aria-expanded={logExpanded}>
              <span>行动记录</span>
              <small>{logExpanded ? "收起 ▴" : `共 ${game.logs.length} 条 · 展开 ▾`}</small>
            </button>
            <div className="table-log-list">
              {[...game.logs].reverse().slice(0, logExpanded ? 8 : 2).map((log) => (
                <p className={log.kind} key={log.id}>
                  <time>{new Date(log.at).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</time>
                  {log.text}
                </p>
              ))}
            </div>
            ...
            {game.phase !== "showdown" && (
              <>
                ...
                <div className="raise-amount-input">
                  <span className="direct-label">直接输入加注到</span>
                  <span className="direct-unit-switch">
                    <button className={amountUnit === "chips" ? "active" : ""} type="button" disabled={!game.validActions.canRaise} onClick={() => selectAmountUnit("chips")}>筹码</button>
                    <button className={amountUnit === "bb" ? "active" : ""} type="button" disabled={!game.validActions.canRaise} onClick={() => selectAmountUnit("bb")}>BB</button>
                  </span>
                  <span className="direct-amount-field">
                    <input type="number" min="0" step={amountUnit === "bb" ? 0.5 : 1} value={directInput} disabled={!game.validActions.canRaise} onChange={(event) => setDirectAmount(event.target.value)} aria-label="直接输入加注金额，可填筹码或 BB" />
                    <b>{amountUnit === "bb" ? "BB" : "筹码"}</b>
                  </span>
                  <small>加注到 {formatAmount(raiseAmount, game.room.bigBlind, amountUnit)}（范围 {formatAmount(game.validActions.minRaiseTo ?? 0, game.room.bigBlind, amountUnit)} ~ {formatAmount(Math.max(game.validActions.minRaiseTo ?? 0, game.validActions.maxRaiseTo ?? 0), game.room.bigBlind, amountUnit)}）</small>
                </div>
          ...
          <div className="table-chat">
            <div className="section-title"><span>牌桌聊天</span><small>跨设备同步</small></div>
            <div className="chat-body">
              <div className="chat-messages">
                {game.chats.length ? game.chats.map((item) => (
                  <div className="chat-message" key={item.id}>
                    <span className="chat-avatar" style={{ background: seatColors[Math.abs(item.userId.length) % seatColors.length] }}>{initials(item.name).slice(0, 1)}</span>
                    <div>
                      <span className="chat-meta"><b>{item.name}</b><time>{new Date(item.at).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</time></span>
                      <p>{item.text}</p>
                    </div>
                  </div>
                )) : <p className="empty-chat">还没有消息，先打个招呼吧。</p>}
              </div>
              <form className="chat-form" onSubmit={sendMessage}>
                <input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="说点什么…" maxLength={120} aria-label="聊天消息" />

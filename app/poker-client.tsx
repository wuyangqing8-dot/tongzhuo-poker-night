    ...
    <main className="app-shell">
      ...
      <div className="game-layout" ref={layoutRef}>
        ...
        <aside className={`side-panel ${showMobilePanel ? "mobile-panel-open" : ""}`}>
          ...
          <div className="log-card"><button type="button" className="section-title log-card-head" onClick={() => setLogExpanded((value) => !value)} aria-expanded={logExpanded}><span>行动记录</span><small>{logExpanded ? "收起 ▴" : `共 ${game.logs.length} 条 · 展开 ▾`}</small></button><div className="game-logs">{[...game.logs].reverse().slice(0, logExpanded ? 8 : 2).map((log) => <p className={log.kind} key={log.id}><time>{new Date(log.at).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</time>{log.text}</p>)}</div></div>
